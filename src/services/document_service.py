...
class DocumentService:
    ...
    async def upload_document(self, user_id: uuid.UUID, file: UploadFile) -> DocumentEntity:
        from services.ingestion_tasks import process_and_ingest_document # Import inside to avoid circular deps
        ...
        try:
            file_path, file_size, file_hash = await self.storage_service.save_file(file, max_size=settings.MAX_UPLOAD_SIZE)
        ...
        if existing_doc:
            ...
            raise FileExistsError(f"A document with this identical content already exists", existing_doc)
